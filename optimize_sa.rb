...
def optimize_sa(initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
  ...
  min_be, min_pe, min_se, min_ce, min_de, min_fatigue = func(perm_set, initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
  puts "최초 배열의 피로도: %.4f/%.4f/%.4f/%.4f/%.4f/%.4f" % [min_be, min_pe, min_se, min_ce, min_de, min_fatigue]
  ...
  (0...$n).each do |i|
    ...
    be, pe, se, ce, de, fatigue = func(new_perm_set, initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
    ...
    if rand < p
      ...
      min_fatigue = fatigue
      min_be, min_pe, min_se, min_ce, min_de = be, pe, se, ce, de
    end

    puts "#{i}th cycle: #{min_perm_set}/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f, 경과 시간: %d초" % [min_be,
                                                                                 min_pe,
                                                                                 min_se,
                                                                                 min_ce,
                                                                                 min_de,
