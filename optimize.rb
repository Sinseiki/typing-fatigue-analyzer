...
$stored_data_points = []
$best_in_history = [[], [33.0, 33.0, 33.0, 33.0, 33.0, 99.0]]
...
def func(perm_set, initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
  $stored_data_points.each do |data_point|
    if data_point[0] == perm_set
      puts "#{perm_set}: %.4f/%.4f/%.4f/%.4f/%.4f/%.4f" % [data_point[1][0],
                                                 data_point[1][1],
                                                 data_point[1][2],
                                                 data_point[1][3],
                                                 data_point[1][4],
  ...
  fatigue = (be + pe + se + ce) * analysis.count_strokes / total_jamo + de

  #print perm_set
  puts "#{perm_set}: %.4f/%.4f/%.4f/%.4f/%.4f/%.4f" % [be, pe, se, ce, de, fatigue]

  $stored_data_points << [perm_set, [be, pe, se, ce, de, fatigue]]

  if fatigue < $best_in_history[1][-1]
    $best_in_history = [perm_set, [be, pe, se, ce, de, fatigue]]
  end

  return [be, pe, se, ce, de, fatigue]
end
...
def optimize(initial_layout, jamo_data, scheme)
  ...
  puts "각 피로도 항목: %.4f/%.4f/%.4f/%.4f/%.4f/%.4f" % [be, pe, se, ce, de, fatigue]
