...
def optimize_crs(initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
  ...
  be, pe, se, ce, de, fatigue = func(perm_set, initial_layout, cho_opt, jung_opt, jong_opt, jamo_data)
  puts "최초 배열의 피로도: %.4f/%.4f/%.4f/%.4f/%.4f/%.4f" % [be, pe, se, ce, de, fatigue]
  ...
  data_set.add_data(initial_perm_set, [be, pe, se, ce, de, fatigue])
  ...
  (0...$max_cycle * set_size).each do |i|
    ...
    if (i % 10 == 0) && (i > 0)
      best = data_set.best
      puts "#{i}th cycle: #{best.perm_set}/%.4f/%.4f/%.4f/%.4f/%.4f/%.4f, 경과 시간: %d초" % [best.vals[0],
                                                                                    best.vals[1],
                                                                                    best.vals[2], 
                                                                                    best.vals[3],
                                                                                    best.vals[4],
